 ...
 *
 * Week 2 features already working:
 *   ✅ Local video / audio with toggle controls
 *   ✅ Screen sharing
 *   ✅ Basic UI shell
 *
 * What this file adds (Week 2/3 bridge):
 *   🔗 Real Socket.io chat (replaces the local-only useState chat)
 *   🔗 useWebRTC hook wired in (replaces bare getUserMedia call)
 *   🔗 Remote participant video tiles rendered from hook state
 *   🔗 Live transcript panel (from Web Speech API inside hook)
 *   🔗 "End Meeting" → POSTs transcript to /api/meetings/:id/end-meeting
 *      which triggers server-side AI summarisation (Week 3 Day 16-17)
 */
...
await fetch(`/api/meetings/${id}/end-meeting`, {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col relative select-none">
      ...
      {/* ── TRANSCRIPT PANEL (Week 3 bridge) ───────────────────────────────── */}
      {showTranscript && (
