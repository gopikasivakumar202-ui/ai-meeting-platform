...
export default function MeetingRoomPage() {
  ...
  const {
    localStream, peers, videoOn, audioOn,
    messages, transcript, toggleVideo, toggleAudio,
    shareScreen, sendMessage, leaveMeeting,
  } = useWebRTC(
    id ?? '',
    userLoaded ? (user?._id ?? '') : '',
    user?.name ?? 'Guest',
  ...
  if (!userLoaded) {
    return <div style={{color:'white', display:'flex', alignItems:'center', justifyContent:'center', height:'100vh', background:'#050810'}}>⏳ Loading...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col relative select-none">
        ...
        <div className={`grid gap-4 h-full ${
          ...
          {peers.map(peer =>
            peer.stream ? (
              ...
              <div key={peer.userId} className="bg-gray-900 border border-gray-700 rounded-2xl aspect-video flex items-center justify-center">
      ...
      {showChat && (
        <div className="absolute right-0 top-16 w-80 h-[calc(100%-8rem)] bg-gray-800 border-l border-gray-700 flex flex-col shadow-2xl z-10">
          ...
          <div className="px-3 py-3 border-t border-gray-700 flex gap-2">
            ...
            />
            <button onClick={handleSend} className="bg-blue-600 hover:bg-blue-700 text-white px-3 rounded-lg text-sm">↩</button>
          </div>
        </div>
      )}
      ...
      <div className="bg-gray-900 border-t border-gray-800 px-6 py-4 flex justify-center gap-3">
        <ControlBtn onClick={toggleAudio} active={audioOn} emoji="🎤" label={audioOn ? 'Mute' : 'Unmute'} />
        <ControlBtn onClick={toggleVideo} active={videoOn} emoji="📹" label={videoOn ? 'Stop Video' : 'Start Video'} />
        <ControlBtn onClick={shareScreen} active={true} emoji="🖥️" label="Share Screen" />
        <ControlBtn onClick={() => { setShowChat(s => !s); setShowTranscript(false); }} active={!showChat} emoji="💬" label="Chat" badge={messages.length > 0 ? messages.length : undefined} />
        <ControlBtn onClick={() => { setShowTranscript(s => !s); setShowChat(false); }} active={!showTranscript} emoji="📝" label="Transcript" />
        <ControlBtn onClick={() => { leaveMeeting(); navigate('/dashboard'); }} active={false} emoji="📵" label="Leave" danger />
      </div>
    </div>
  );
}

function ControlBtn({ onClick, active, emoji, label, danger, badge }: {
  onClick: () => void; active: boolean; emoji: string; label: string; danger?: boolean; badge?: number;
}) {
  return (
    <button onClick={onClick} title={label} className={`relative w-12 h-12 rounded-full text-xl transition-colors ${
